...
test.out<- compareCluster(geneCluster=inputList, fun="GSEA", pvalueCutoff=0.05,
                          ...
                          seed = T)
saveRDS(test.out, "2D_NPC_apex_vs_correction_comparecluster_Tx_10_500.rds")
test.out <- readRDS("2D_NPC_apex_vs_correction_comparecluster_Tx_10_500.rds")

dotplot(test.out, showCategory=6, split=".sign", label_format = 60) + facet_grid(.~.sign) + theme(axis.text.x=element_text(angle=45, hjust=1)) 